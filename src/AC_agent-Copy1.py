...
class Actor_critic(nn.Module):
    ...
    def __init__(self):
        ...
        self.act_linear = nn.Linear(256, engine.nb_actions)
...
def select_action(state):
    ...
    if sample > eps_threshold:
        Q, act_prob = model(Variable(state, volatile=True)
        act_prob = act_prob.cpu().numpy().flatten()
        act = np.random.choice(np.arange(engine.nb_actions), act_prob)
        return act
    else:
